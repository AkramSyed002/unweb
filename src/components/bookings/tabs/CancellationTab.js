...
export default function CancellationTab({ title, count, allBookings }) {
  ...
  // MY WORK
  const onBookingStatusChange = async (e) => {
    let tempRecord = Object.assign({}, selectedRecord);
    tempRecord.status = e.target.value;
    tempRecord.last_updated_at = new Date();
    setLoading(true);
    try {
      await updateBooking(tempRecord);
      const temp = [...bookings];
      const index = temp.findIndex((el) => el.id === tempRecord.id);
      temp[index] = tempRecord;
      setBookings(temp);
      setSelectedRecord(null);
      setLoading(false);
    } catch (error) {
